...
if selected == "Dashboard":
    ...
    with right_col1:
        ...
        fig2 = px.pie(data_frame=area_wise_freq,names='Area',values='Percentage of Fault freq',hole=0.5, hover_data='Frequency')
    ...
    with left_col2:
        ...
        fig3 = px.bar(data_frame=monthwise_fault, x= 'Month', y = ['Frequency','WEC'],barmode='group',height= 500, text_auto=True)
        st.plotly_chart(fig3, use_container_width=True)
    
    # create pie chart for Top 5 high fault freq
    with right_col2:
        st.subheader('Top 5 High Fault Freq Wec')
        fig4 = px.pie(data_frame=wec_wise_freq, names= 'WEC', values= 'Frequency', hole=0.5, height=500, template="plotly_dark" )
    ...
    with left_col3:
        ...
        fig5 = px.bar(status_wise_freq, x= 'Frequency', y= 'StatusCode', text=['{:,}'.format(x) for x in status_wise_freq['Frequency']], height=500)
    ...
    with right_col3:
        ...
        fig6 = px.pie(data_frame=top_5_high_duration_wec,values='Total_hours', names='WEC', hole=0.5, height=500, template="plotly_white" , hover_data=['StatusCode'])
