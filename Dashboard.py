...
def read_excel_file():
    ...
    convert_dt = {'Main code' : int, 'sub code' : int, 'StatusCode' : str}
...
if selected == "Dashboard":
    ...
    with state:
        state= st.multiselect('Select State', options= df['State'].unique())
    if not state:
        df2 = df.copy()
    else:   
    ...
    state_wise_freq = filtered_df.groupby(by='State',as_index=False)['Frequency'].sum().sort_values('State',ascending=True)
...
#     # Grouping Month wise fault freq and wec count
    ...
    month_mapping = { 'Jun': 1,'Jul': 2, 'Aug': 3, 'Sept': 4, 'Oct' :5 , 'Nov': 6}
...
#     # Repeated fault continue 3 month
    ...
    repeated_fault_df = df2.pivot_table(index=['State','Area','WEC','StatusCode'],columns='Month', values='Frequency', aggfunc='sum', margins=True, margins_name='Sum of Frequency').sort_values('Sum of Frequency', ascending=False).iloc[1:]
...
#     #  Creating combination for conitinuos repeated error
    ...
    repeated_fault_df['Fault repeated continue > 3 months'] = np.where((
        ...
        ((repeated_fault_df['Jun']>0) & (repeated_fault_df['Jul']>0) & (repeated_fault_df['Aug']>0) & (repeated_fault_df['Sept']>0) & (repeated_fault_df['Oct']>0) & (repeated_fault_df['Nov']>0))),'repeated','not repeated')
...
#     #  Creating Bar Charts for state wise freq
    ...
    with right_col1:
        ...
        #  Creating Bar Charts for Areawise wise freq

        st.subheader('Top 5 High Fault freq Areawise')
        ...
        st.plotly_chart(fig2,use_container_width=True)
         ...
         #  download data for areawise wise freq
...
#     #  Creating bar graph for Monthwise fault freq and fault count
...
#     #  Creating pie chart for Top 5 High Fault Freq Wec
    with right_col2:
...
#     #  Creating bar graph Top 5 High Fault Frequ Status code and download file
    with left_col3:
...
#      #  Creating pie chart Top 5 High downtime Wec and download file
...
#     #  download file for Top 10 wec repeated fault continue for > 3 months
